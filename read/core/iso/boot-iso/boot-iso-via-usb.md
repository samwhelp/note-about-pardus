...
``` sh
wget -c https://indir.pardus.org.tr/ISO/Pardus25/Pardus-25.0-XFCE-amd64.iso
```

就會下載一個檔案「`Pardus-25.0-XFCE-amd64.iso`」。
...
``` sh
sudo dd if=./Pardus-25.0-XFCE-amd64.iso of=/dev/sdc bs=4M status=progress && sync
```

或是執行下面指令，就會寫入「USB隨身碟」。

``` sh
sudo dd if=./Pardus-25.0-XFCE-amd64.iso of=/dev/sdc bs=4M status=progress conv=fsync oflag=direct
```
